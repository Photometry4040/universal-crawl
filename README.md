...
4. 툴바에 보라색 아이콘이 생기면 클릭해 팝업을 엽니다.
...
1. **대상 페이지**를 엽니다(예: `https://quotes.toscrape.com`). 아이콘을 눌러 팝업을 엽니다.
2. **섹션 1 대상 확인**: robots.txt 상태(🟢/🔴/🟠)를 확인하고, ToS 체크박스를 선택합니다. 🔴/🟠면 경고 배너의
   "이해했고 계속한다"를 눌러야 진행됩니다.
3. **섹션 2 행 선택**: "행 선택 모드 시작"을 누르면 팝업이 닫히고 페이지에서 선택 모드가 켜집니다.
   수집할 항목(예: 인용구 카드)을 **2개 클릭**하면 공통 셀렉터가 추론되고 매칭 요소가 주황색으로 미리보기됩니다.
   `ESC`로 종료한 뒤 팝업을 다시 열면 `row_selector`와 매칭 개수가 복원됩니다.
...
7. **섹션 6 결과**: 상위 10행 미리보기 후 CSV/JSON 다운로드. 현재 설정은 "프로필 export"로 저장하고 다른 세션에서 import할 수 있습니다.
