...
> 🖥️ **크롬 확장을 못 쓰는 환경(회사 보안 등)이면 → [`desktop/`](desktop/README.md)** 의 데스크탑 앱(Tauri)을 쓰세요.
> 같은 기능을 브라우저 확장 없이 제공하며, 항목 1회 클릭으로 컬럼을 자동 추출합니다(`cd desktop && npm install && npm run tauri dev`).
...
보안·권한·데이터 처리·책임 있는 사용은 [`SECURITY.md`](SECURITY.md) 참고.

---

## 문제 해결 (Troubleshooting / 디버그)

### "현재 페이지 추출" / "N페이지 수집" 버튼이 비활성화됨
추출 게이트의 **4개 조건**이 모두 충족돼야 활성화됩니다. **섹션5 버튼 아래 회색 안내문(hint)** 이 무엇이 부족한지 알려줍니다:
1. **ToS 체크박스**(섹션1) 선택
2. **robots.txt** 가 🟢 이거나, 🔴/🟠면 경고 배너의 **"이해했고 계속한다"** 클릭
3. **`row_selector`**(섹션2)에 값 있음
4. **필드 1개 이상**(섹션3)
- 이전 사이트에서 "N페이지 수집"이 끝나지 않고 남아 있으면 `running` 상태로 인식돼 버튼이 잠깁니다 → **"중단"** 버튼을 누르세요(없으면 확장 새로고침).

### robots.txt가 🔴 Disallow로 떠서 막힘
**버그가 아니라 정상**입니다. 해당 사이트가 그 경로의 자동 수집을 **명시적으로 금지**한 것입니다(예: `Disallow: /path?`는 쿼리가 붙은 페이지 차단). 사이트 정책을 존중하세요. 학습은 연습 사이트로.

### 유사 요소가 안 잡히거나 매칭 개수가 이상함
- **난수/해시 클래스 사이트**(CSS-in-JS, `css-1a2b3c` 등): 셀렉터가 일반화되지 않습니다. finder가 이런 클래스를 거르지만, 한계가 있어 **`row_selector`를 수동 입력**하는 게 빠릅니다.
- **가상 스크롤/동적 로드**(뉴스 포털 등): 스크롤해야 카드가 로드되거나 DOM이 자주 바뀌어 선택이 불안정합니다. `infinite_scroll` 모드를 시도하거나 수동 셀렉터 사용.
- **빽빽한 카드**: 깊은 하위 요소를 클릭해도 자동으로 **반복 카드 컨테이너로 스냅**되지만, 광고/구독 헤더가 섞인 다단 레이아웃은 정확도가 떨어집니다.

### 다운로드가 안 됨
- 먼저 **"현재 페이지 추출"** 또는 **"N페이지 수집"** 으로 데이터를 만든 뒤 다운로드하세요(데이터가 없으면 "no data").
- **저장 폴더**(섹션6)를 비우면 다운로드 폴더로 바로 저장됩니다. "매번 위치 묻기"를 켜면 위치를 선택합니다. 다운로드 상태는 버튼 아래에 표시됩니다.

### 사이드 패널이 안 보임 / 변경이 반영 안 됨
- 툴바 아이콘을 클릭하면 오른쪽에 열립니다(Chrome 114+).
- 코드 수정 후 `chrome://extensions` → 확장 카드 **새로고침(↻)**, content script 변경 시 대상 페이지도 새로고침.

### 디버깅 위치
- **사이드 패널**: 패널 우클릭 → "검사".
- **서비스 워커**: `chrome://extensions` → 확장 카드의 "service worker" 링크. 라이프사이클은 `chrome://serviceworker-internals`.
- **content script**: 대상 페이지의 DevTools 콘솔.
- **자동 검증**: `cd e2e && npm test`([e2e/README.md](e2e/README.md)).
