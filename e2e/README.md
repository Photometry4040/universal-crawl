...
### `ux.js` — UX 상호작용 15 PASS (헤디드)
- **시각적 행 선택**: 모드 배지 표시 → 실제 클릭 2회 → 셀렉터 추론(`span.text`)·10개 매칭·미리보기 하이라이트 → ESC 종료
- **시각적 필드 선택**: 확정 행 하이라이트 → 행 내부 클릭 → 상대 셀렉터(`.author`)
- **실제 다운로드**: 1페이지 수집 → `exportCsv` → 저장 파일에 UTF-8 BOM·CSV 헤더·10행 데이터 확인
